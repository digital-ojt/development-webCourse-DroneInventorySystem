...
public abstract class AbstractController implements ErrorController {
  ...
  @Autowired private HttpServletRequest request; // リクエスト情報を取得
  ...
  protected void logStart(String action) {
    ...
    logger.info(
        String.format(LogMessage.APP_LOG, action, getMethodName(), LogMessage.PROCESS_START));
  }
  ...
  protected void logException(String action, String errorMsg) {
    logger.error(
        String.format(LogMessage.ERROR_LOG, action, getMethodName(), String.valueOf(errorMsg)));
  }
  ...
  public void setFlashErrorMsg(
      MessageSource messageSource, RedirectAttributes redirectAttributes, String messageConst) {
